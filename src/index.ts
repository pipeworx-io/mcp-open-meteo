...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Open-Meteo');
}
...
async function geocode(args: Record<string, unknown>) {
  ...
  const requestedCount = Math.min(100, Math.max(1, (args.count as number) ?? 10));
  const language = String(args.language ?? 'en');
  // fetched 10 candidates and sorted by population, under a comment saying that
  // "returns the right city". It does not, and the live proof is the exact
  // example that comment used: `Roma` came back as Roma, LESOTHO (pop 14,259).
  // Rome is ABSENT from Open-Meteo's ENGLISH results entirely, so no amount of
  // re-sorting an English result set can surface it — the query has to be
  // reissued in Italian. Population sort is necessary and not sufficient.
  const hits = await rankedPlaceHits(name, { language, count: requestedCount });
  // Upstream's own object shape is preserved: callers of this tool read
  // Open-Meteo fields directly, so normalising here would be a breaking change.
  return { results: hits.slice(0, requestedCount) };
}
...
async function meteoGet(url: string, params: URLSearchParams) {
  const res = await pwFetch(`${url}?${params}`, { headers: { Accept: 'application/json' } });
